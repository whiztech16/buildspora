...
function Input({ label, id, placeholder, type = "text", required, maxLength, hint }: {
...
function Textarea({ label, id, placeholder, maxLength }: {
  ...
  return (
    <div>
      ...
      />
      {maxLength && (
        <p className="text-[11.5px] text-[#94A3B8] text-right mt-1" style={{ fontFamily: FONT }}>{count}/{maxLength}</p>
      )}
    </div>
  );
}

function Select({ label, id, options, placeholder, required }: {
  label: string; id: string; options: string[]; placeholder?: string; required?: boolean;
}) {
  return <SearchableSelect id={id} label={label} options={options} placeholder={placeholder ?? "Search or select…"} required={required} />;
}
...
function Step1() {
  ...
  const cities = ALL_CITIES[selectedState] ?? ALL_CITIES["Default"];
  ...
  return (
    <div className="space-y-5">
      ...
      <Textarea id="bizDesc" label="Business Description" placeholder="e.g. We supply quality building materials across Lagos and Ogun State at competitive prices." maxLength={500} />

      {/* State */}
      <SearchableSelect id="state" label="State" required options={NIGERIAN_STATES} placeholder="Search state…" onChange={v => { setSelectedState(v); setSelectedCities([]); }} />

      {/* Cities multi-select */}
      <div>
        <Label text="Cities / Areas Served" required />
        <p className="text-[12px] text-[#94A3B8] mb-2" style={{ fontFamily: FONT }}>
          {selectedState ? `Select areas in ${selectedState}` : "Select a state first"}
        </p>
...
function SupplyTagSelect() {
  const [selected, setSelected] = useState<string[]>([]);
  const toggle = (cat: string) => setSelected(p => p.includes(cat) ? p.filter(c => c !== cat) : [...p, cat]);
  return (
    <div>
    ...
    <div className="space-y-5">
      ...
      {items.map((item, idx) => (
        <div key={item.id} className="p-5 rounded-xl border border-[#E2E8F0] bg-[#FAFAFA] space-y-4">
          ...
          <Input id={`itemName-${item.id}`} label="Item Name" placeholder="e.g. Dangote Cement 50kg" required />
          <div className="grid grid-cols-2 gap-4">
            <Input id={`itemPrice-${item.id}`} label="Unit Price (₦)" placeholder="e.g. 5800" type="text" required />
            <Input id={`itemUnit-${item.id}`} label="Unit" placeholder="e.g. per bag, per tonne" required />
          </div>
          <Input id={`itemSKU-${item.id}`} label="SKU / Product Code" placeholder="e.g. DAN-50KG" />
          <div>
...
function Step3() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Input id="cacNumber" label="CAC Registration Number" placeholder="e.g. RC123456" required />
      <div>
      ...
      <div className="border-t border-[#F1F5F9] pt-5">
        ...
        <div className="space-y-4">
          <Select id="bankName" label="Bank Name" required placeholder="Search bank…" options={NIGERIAN_BANKS} />
          <Input id="accountNumber" label="Account Number" placeholder="10-digit account number" required maxLength={10} />
          <div>
            <Label text="Account Name" />
            <div className="w-full h-[46px] px-4 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] flex items-center">
              <span className="text-[13.5px] text-[#94A3B8]" style={{ fontFamily: FONT }}>Auto-filled after account verification</span>
            </div>
          </div>
        </div>
      </div>
